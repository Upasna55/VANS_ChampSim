...
#nvmem_fmtstr = 'NVDIMM {attrs[name]}("{attrs[config_filename]}", {attrs[frequency]});\n'
nvmem_fmtstr = 'NVDIMM {attrs[name]}("'+os.getcwd()+'/{attrs[config_filename]}", {attrs[frequency]});\n'
...
default_root = { 'executable_name': 'bin/champsim', 'block_size': 64, 'page_size': 4096, 'heartbeat_frequency': 10000000, 'num_cores': 1, 'DIB': {}, 'L1I': {}, 'L1D': {}, 'L2C': {}, 'ITLB': {}, 'DTLB': {}, 'STLB': {}, 'LLC': {}, 'physical_memory': {}, 'virtual_memory': {}, 'non_volatile_memory': {} }
...
default_nvmem = { 'name': 'VANS', 'config_filename': 'config/vans.cfg', 'frequency': 3200 }
...
config_file['non_volatile_memory'] = ChainMap(config_file['non_volatile_memory'], default_nvmem.copy())
...
for cpu in cores:
    ...
    if cache_name != 'VANS' and cache_name != 'DRAM':
...
freqs = list(itertools.chain(
    ...
    (config_file['non_volatile_memory']['frequency'],),
    (config_file['physical_memory']['frequency'],)
))
freqs = [max(freqs)/x for x in freqs]
for freq,src in zip(freqs, itertools.chain(cores, caches.values(), (config_file['non_volatile_memory'],), (config_file['physical_memory'],))):
    src['frequency'] = freq
...
for fill_level in range(1,len(memory_system)+1):
    for k in active_keys:
        if memory_system[k]['lower_level'] != 'VANS' and memory_system[k]['lower_level'] != 'DRAM':
    ...
    active_keys = [memory_system[k]['lower_level'] for k in active_keys if memory_system[k]['lower_level'] != 'VANS' and memory_system[k]['lower_level'] != 'DRAM']
...
for i in reversed(range(len(memory_system))):
    ul = memory_system[i]
    if ul['lower_level'] != 'VANS' and ul['lower_level'] != 'DRAM':
...
with open(instantiation_file_name, 'wt') as wfp:
    ...
    wfp.write('#include "vans_wrapper.h"\n')
    ...
    wfp.write(nvmem_fmtstr.format(attrs=config_file['non_volatile_memory']))
    wfp.write('\n')
    ...
    wfp.write(', '.join('&{name}'.format(**elem) for elem in itertools.chain(cores, memory_system, (config_file['non_volatile_memory'],))))
