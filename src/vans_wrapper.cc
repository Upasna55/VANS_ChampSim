...
int NVDIMM::add_rq( PACKET* packet)
{
    ...
    clk_t curr_clk = this->current_cycle;
    ...
    auto callback = [&, curr_clk](logic_addr_t logic_addr, clk_t clk)
    {
        ...
        for (ptr = outstanding.begin(); ptr < outstanding.end(); ptr++)
            if ((logic_addr >> 6) == (ptr->request->address >> 6))
            {
                ptr->completed = true;
...
void NVDIMM::operate()
{
    ...
    for (ptr = outstanding.begin(); ptr < outstanding.end(); ptr++)
    {
        ...
        {
	    for(auto ret : ptr->request->to_return)
		    ret->return_data(ptr->request);
